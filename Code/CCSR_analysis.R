...
# testing effect of imbalance in number of replicates across richness treatments by capping to max 2 replicates (response to reviewer)
#dat <- dat %>% filter(replicate < 3)

# Report average species sizes observed during experiment in methods
avg_volume <- dat %>% 
...
dat2 <- dat %>% 
  ...
  summarize(mean_major = weighted.mean(mean_major, na.rm=T, w=N), 
            ...
            # scale up from viewing field to total number of individuals per mL at the magnification used
            abundance = sum(N)*13.84/0.6) 

# log-transform size and abundance and encode incubator information for random effects model
dat3 <- dat2 %>%
...
# center predictor variables for easier interpretation and improving model convergence
...
## Understanding size-abundance scaling through time (reproduce Figure 4)
dat2 <- dat %>% 
...
ggsave("output/Figure_S2.png", width=7, height=5)
...
# Figure S5: plot richness x temperature across day 11, 17, 23, 29
...
## Comparing observed and communities with no species interaction expectation
...
# joint figure 4 comparing observed and simulated community size abundance scaling
